...
class Buffer(object):
    ...
    def __init__(self, buffer_size, _buffer=None, last_loss=0):
        ...
        if _buffer is None:
            ...
            self.buffer = OrderedDict(sorted(_buffer.items(), key=lambda obj:obj[1]['loss'], reverse=True))
            self.size = buffer_size
    ...
    def add(self, other_Buffer):
        ...
        self.buffer = OrderedDict(sorted(self.buffer.items(), key=lambda obj:obj[1]['loss'], reverse=True))
    ...
    def _print(self, sample_number=5):
        pass
        # sample_list = random.sample(list(self.buffer.keys()), sample_number)
...
class BufferPool(object):
    ...
    def _print(self):
        ...
        for category in self.buffer_pool.keys():
            ...
            print ('%s : buffer_size [%d], data_size [%d], ratio [%.4f], real_buffer_size [%d], last_loss [%.4f]' %
                   (category, buffer_size, data_size, ratio, real_buffer_size, last_loss))
