...
class JointMatching(nn.Module):
  ...
  def __init__(self, opt):
    ...
    """
    self.sub_matching = Matching(opt['cemb_dim'], opt['cemb_dim'],
  ...
  def forward(self, pool5, fc7, lfeats, dif_lfeats, cxt_fc7, cxt_lfeats, labels):
    ...
    """
    sub_feats, sub_grid_attn = self.sub_encoder(fc7, lfeats, hidden) # (n, fc7_dim+att_dim), (n, 49), (n, num_atts)
    sub_matching_scores = self.sub_matching(sub_feats, hidden) # (n, 1)
    """
    ...
    """
    scores = sub_matching_scores.view(-1)
    return scores, sub_grid_attn
    """
