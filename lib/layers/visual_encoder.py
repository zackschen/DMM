...
class SubjectEncoder(nn.Module):
  ...
  def __init__(self, opt):
    ...
    """
    self.fc7_normalizer   = Normalize_Scale(opt['cemb_dim'],   opt['visual_init_norm'])
    """
    ...
    """
    self.phrase_normalizer= Normalize_Scale(opt['cemb_dim'], opt['visual_init_norm'])
    """
    ...
    self.attn_fuse = nn.Sequential(nn.Linear(opt['fc7_dim']+opt['jemb_dim']+opt['word_vec_size'],
                                             opt['jemb_dim']),
                                   nn.Tanh(),
    ...
    self.img_map = nn.Sequential(nn.Conv2d(opt['fc7_dim'], opt['cemb_dim'], 3, padding=1),
                                 nn.LeakyReLU(negative_slope=0.25))
    self.attn_fuse = nn.Sequential(nn.Linear(opt['cemb_dim']+opt['cemb_dim']+opt['loc_dim'], opt['jemb_dim']),
                                   nn.Tanh(),
                                   nn.Linear(opt['jemb_dim'], 1))
    """
  ...
  # def forward(self, fc7, lfeats, phrase_emb):
    """Inputs
    - pool5     : (n, 1024, 7, 7)
    - fc7       : (n, 2048, 7, 7)
    - lfeats    : (n, 5)
    ...
    """
    """
    batch, grids = fc7.size(0), fc7.size(2)*fc7.size(3)
    """
    ...
    """
    fc7 = self.img_map(fc7)  # (n, 1024, 7, 7)
    fc7 = fc7.view(batch, self.cemb_dim, -1)  # (n, 2048, 49)
    fc7 = fc7.transpose(1,2).contiguous().view(-1, self.cemb_dim) # (n x 49, 2048)
    fc7 = self.fc7_normalizer(fc7) # (nx49, 2048)
    """
    # att_feats
    ...
    """
    visual_feats = fc7  # (nx49, 1024)
    phrase_emb = self.text_map(phrase_emb)
    phrase_emb = self.phrase_normalizer(phrase_emb)# (n, word_vec_size)
    phrase_emb = phrase_emb.unsqueeze(1).expand(batch, grids, self.cemb_dim) # (n, 49, word_vec_size)
    phrase_emb = phrase_emb.contiguous().view(-1, self.cemb_dim) # (nx49, word_vec_size)
    lfeats = lfeats.unsqueeze(1).expand(batch, grids, 5)
    lfeats = lfeats.contiguous().view(-1, 5)
    attn = self.attn_fuse(torch.cat([visual_feats, lfeats, phrase_emb], 1)) # (nx49, 1)
    """
    ...
    # return weighted_visual_feats, attn
