...
2. anns        : list of {ann_id, category_id, image_id, box, h5_id, kld_label, pos_id, gt_pos_all,
                          bbx_reg, bbx_reg_all}
3. sentences   : list of {sent_id, ref_id, sen_id, category_id, tokens, first_idx, idx, h5_id}
...
class GtMRCNFlickrLoader(Loader):

  def __init__(self, data_json, data_h5, data_split, det_feats_dir, type_split=None, flag_split=None):
    # parent loader instance
    Loader.__init__(self, data_json, data_h5)

    self.det_feats_dir = det_feats_dir
    # img_iterators for each split
    self.split_info = json.load(open(data_split))
    self.Images_Split = {split['image_id']:split['split'] for split in self.split_info}
    self.split_ix = {}
    self.iterators = {}
    for image_id, image in self.Images.items():
      # we use its ref's split (there is assumption that each image only has one split)
      split = self.Images_Split[image_id]
    ...
    self.split_ix_ref = {}
    self.split_category = {}
    for k in self.split_ix:
      split_ref = k + '_ref'
      for image_id in self.split_ix[k]:
        ref_ids = self.Images[image_id]['ref_ids']
        for ref_id in ref_ids:
          if split_ref not in self.split_ix_ref:
            self.split_ix_ref[split_ref] = []
            self.iterators[split_ref] = 0
          self.split_ix_ref[split_ref] += [ref_id]

          category_id = self.Refs[ref_id]['category_id']
          catagory = self.ix_to_cat[category_id]
          split = k + '_' + catagory
          if split not in self.split_category:
            self.split_category[split] = []
            self.iterators[split] = 0
          self.split_category[split] += [ref_id]

    for k, v in self.split_ix.items():
      print('assigned %d images to split %s' % (len(v), k))
    for k, v in self.split_category.items():
      print('assigned %d refs to split %s' % (len(v), k))
  ...
  def getCategoryBatch(self, split, opt):
    # options
    ref_batch_size = opt.get('ref_batch_size', 30)
    seq_per_ref = opt.get('seq_per_ref', 3)
    sample_ratio = opt.get('visual_sample_ratio', 0.3)  # sample ratio, st vs dt
    split_category = self.split_category[split]
    max_index = len(split_category) - 1                       # don't forget to -1
    wrapped = False
  ...
  def getCrossBatch(self, split, opt):
    # options
    ref_batch_size = opt.get('cross_batch_size', 40)
    seq_per_ref = opt.get('cross_seq_per_ref', 1)
    split_ix_ref = self.split_ix_ref[split]
    max_index = len(split_ix_ref) - 1                       # don't forget to -1
    wrapped = False
    ...
    # fetch feats
    batch_ref_ids = []
    batch_ann_ids, batch_category_ids, batch_sent_ids = [], [], []
    batch_det_pool5, batch_det_fc7, batch_det_lfeats = [], [], []
    for image_id in batch_image_ids:
      ref_ids = Images[image_id]['ref_ids']
      ref_ids = self.fliter_ref_ids(ref_ids)
      batch_ref_ids += self.expand_list(ref_ids, seq_per_ref)

      # fetch head and im_info
      det_pool5, det_fc7, det_lfeats = self.image_to_det(image_id, opt) # (num_props, k, 7, 7) (num_props, 5)
      det_pool5 = Variable(torch.from_numpy(det_pool5).float().cuda())
      det_fc7 = Variable(torch.from_numpy(det_fc7).float().cuda())
      det_lfeats = Variable(torch.from_numpy(det_lfeats).float().cuda())
      ...
      for ref_id in ref_ids:
        ...
        ann_ids = [ref_ann_id] * seq_per_ref
        category_ids = [category_id] * seq_per_ref

        sent_ids = self.fetch_sent_ids_by_ref_id(ref_id, seq_per_ref)

        # add to image and batch
        image_ann_ids += ann_ids
        batch_category_ids += category_ids
        batch_sent_ids += sent_ids

        batch_det_pool5.append(det_pool5)
        batch_det_fc7.append(det_fc7)
        batch_det_lfeats.append(det_lfeats)

      # add to batch
      batch_ann_ids += image_ann_ids

    # get feats and labels
    det_fc7 = torch.cat(batch_det_fc7, 0);
    det_pool5 = torch.cat(batch_det_pool5, 0); det_pool5.detach()
    det_lfeats = torch.cat(batch_det_lfeats, 0); det_lfeats.detach()
    labels = np.vstack([self.fetch_seq(sent_id) for sent_id in batch_sent_ids])
    bbx_labels, kld_labels, bbx_reg, reg_inweight, reg_all = self.fetch_det_ann(batch_ann_ids, opt)

    # convert to Variable
    labels = Variable(torch.from_numpy(labels).long().cuda())
    kld_labels = Variable(torch.from_numpy(kld_labels).float().cuda())
    bbx_reg = Variable(torch.from_numpy(bbx_reg).float().cuda())
    reg_inweight = Variable(torch.from_numpy(reg_inweight).float().cuda())
    reg_all = Variable(torch.from_numpy(reg_all).float().cuda())

    # chunk labels using max_len
    ...
    data['ref_category_ids'] = batch_category_ids
    data['ref_sent_ids'] = batch_sent_ids
    data['Feats'] = {'fc7': det_fc7, 'pool5': det_pool5, 'lfeats': det_lfeats}
    data['labels'] = labels
    data['bbx_labels'] = bbx_labels
    data['kld_labels'] = kld_labels
    data['bbx_reg'] = bbx_reg
    data['reg_inweight'] = reg_inweight
    data['reg_all'] = reg_all
    data['bounds'] = {'it_pos_now': self.iterators[split], 'it_max': max_index, 'wrapped': wrapped}
  ...
  def category_count(self, category_batch):
    num_category = np.zeros(len(self.cat_to_ix))
    for cur_category in category_batch:
      num_category[cur_category] = num_category[cur_category] + 1
    return num_category

  def fliter_ref_ids(self, ref_ids):
    ref_ids_temp = ref_ids
    ref_ids = []
    for ref_id in ref_ids_temp:
      ann_id = self.Refs[ref_id]['ann_id']
      pos_id = self.Anns[ann_id]['pos_id']
      if pos_id != -1:
          ref_ids += [ref_id]
    return ref_ids

  def get_reg_inweight(self, pos_all, pos_reg_all, num_props):
    reg_inweight = np.zeros(num_props)
    reg_all = np.zeros((num_props, 4))
    for i in range(len(pos_all)):
        reg_inweight[pos_all[i]] = 1
        reg_all[pos_all[i], :] = pos_reg_all[i]
    return reg_inweight, reg_all

  def fetch_det_ann(self, ann_ids, opt):
    batch_size = len(ann_ids)
    num_props = opt.get('num_props', 100)
    bbx_labels = np.zeros(batch_size).astype('int')
    kld_labels = np.zeros((batch_size, num_props)).astype('float')
    bbx_regs = np.zeros((batch_size, 4)).astype('float')
    reg_inweights = []
    reg_alls = []
    for i, ann_id in enumerate(ann_ids):
      bbx_labels[i] = self.Anns[ann_id]['pos_id']
      kld_labels[i] = self.Anns[ann_id]['kld_label']
      bbx_regs[i] = self.Anns[ann_id]['bbx_reg']

      pos_all = self.Anns[ann_id]['gt_pos_all']
      pos_reg_all = self.Anns[ann_id]['bbx_reg_all']
      reg_inweight, reg_all = self.get_reg_inweight(pos_all, pos_reg_all, num_props)
      reg_inweights.append(reg_inweight)
      reg_alls.append(reg_all)

    reg_inweights = np.array(reg_inweights).astype('int')
    reg_alls = np.array(reg_alls).astype('float')
    return bbx_labels, kld_labels, bbx_regs, reg_inweights, reg_alls
  ...
  def image_to_det(self, image_id, opt):
    """returns
    pool5    (num_props, 1024, 7, 7)
    fc7      (num_props, 2048, 7, 7)
    bbx_data (num_props, 5)
    """
    num_props = opt.get('num_props', 100)
    pool5_data = np.zeros((num_props, 1024, 7, 7)).astype('float')
    fc7_data = np.zeros((num_props, 2048, 7, 7)).astype('float')
    bbx_data = np.zeros((num_props, 5))

    h5_data = h5py.File('%s/%d.h5'%(self.det_feats_dir, image_id), 'r')
    bbox = h5_data['bbox']
    w, h = self.Images[image_id]['width'], self.Images[image_id]['height']

    pool5_data[:len(bbox)] = h5_data['pool5']
    fc7_data[:len(bbox)] = h5_data['fc7']
    bbx_data[:bbox.shape[0], 0] = bbox[:, 0] / w
    bbx_data[:bbox.shape[0], 1] = bbox[:, 1] / h
    bbx_data[:bbox.shape[0], 2] = bbox[:, 2] / w
    bbx_data[:bbox.shape[0], 3] = bbox[:, 3] / h
    bbx_data[:bbox.shape[0], 4] = \
            (bbox[:, 2]-bbox[:, 0]+1.0)*(bbox[:, 3]-bbox[:, 1]+1.0)/w/h

    return pool5_data, fc7_data, bbx_data
  ...
  def getTestCrossBatch(self, split, opt):
    # Fetch feats according to the image_split_ix
    # current image
    wrapped = False
    split_ix = self.split_ix[split]
    ...
    bbx_loc = np.array(image['det_bbox']).astype('float')

    # fetch head and im_info
    # (num_props, k, 7, 7) (num_props, 5)
    det_pool5, det_fc7, det_lfeats = self.image_to_det(image_id, opt)
    det_pool5 = Variable(torch.from_numpy(det_pool5).float().cuda())
    det_fc7 = Variable(torch.from_numpy(det_fc7).float().cuda())
    det_lfeats = Variable(torch.from_numpy(det_lfeats).float().cuda())
    ...
    for ref_id in image['ref_ids']:
      ...
      for sent_id in ref['sent_ids']:
        ...
        gt_pos_all.append(self.Anns[ann_id]['gt_pos_all'])
        gt_loc_all.append(self.Anns[ann_id]['box'])
        pos_id = self.Anns[ann_id]['pos_id']
        if pos_id != -1:
    ...
    gt_loc_all = xywh_to_xyxy(np.array(gt_loc_all))

    labels = np.vstack([self.fetch_seq(sent_id) for sent_id in sent_ids])
    num_category = self.category_count(category_ids)

    # move to Variable
    det_fc7   = torch.cat(batch_det_fc7, 0); det_fc7.detach()
    det_pool5 = torch.cat(batch_det_pool5, 0); det_pool5.detach()
    det_lfeats = torch.cat(batch_det_lfeats, 0); det_lfeats.detach()
    labels = Variable(torch.from_numpy(labels).long().cuda())
    ...
    data['Feats'] = {'fc7': det_fc7, 'pool5': det_pool5, 'lfeats': det_lfeats}
    data['labels'] = labels
    data['gt_pos_all'] = gt_pos_all
    data['gt_loc_all'] = gt_loc_all
    data['bbx_loc'] = bbx_loc
    data['num_sample'] = num_sample
    data['pos_or_reg'] = pos_or_reg
    data['h'] = h
    data['w'] = w
    data['bounds'] = {'it_pos_now': self.iterators[split], 'it_max': max_index, 'wrapped': wrapped}
    return data
