...
class GtMRCNLoader(Loader):
  ...
  def __init__(self, data_json, data_h5, opt):
    ...
    self.test_iterators = 0
    self.test_ix = []

    self.isDebug = True if sys.gettrace() else False
  ...
  def getCategoryBatch(self, split, opt, chunk_flag=True):
    ...
    category = split.split('_')[-1]
    wrapped = False

    index = 100 if self.isDebug else max_index
  ...
  def gen_cl_mapping(self, buffer_size, task_count):
    total_len = 0
    mem_per_task = buffer_size//task_count
    new_split_supercategory_ix = {}
    for k,v in self.split_supercategory_ix.items():
      new_data = v
      if 'train' in k and not 'img' in k:
        random.shuffle(v)
        new_data = v[:mem_per_task]
      new_split_supercategory_ix[k] = new_data
    self.split_supercategory_ix = new_split_supercategory_ix
  ...
  def getUnionTestBatch(self, opt):
    # Fetch feats according to the image_split_ix
    # current image
    wrapped = False
    split_ix = self.test_ix
    max_index = len(split_ix) - 1
    ri = self.test_iterators
    ri_next = ri+1
    if ri_next > max_index:
      ri_next = 0
      wrapped = True
    self.test_iterators = ri_next
    image_id = split_ix[ri]
    image = self.Images[image_id]
    ...
    data['Feats']  = {'pool5': pool5, 'fc7': fc7, 'lfeats': lfeats, 'dif_lfeats': dif_lfeats,
                      'cxt_fc7': cxt_fc7, 'cxt_lfeats': cxt_lfeats}
    data['labels'] = labels
    data['bounds'] = {'it_pos_now': self.test_iterators, 'it_max': max_index, 'wrapped': wrapped}
    return data
