...
from mas.mas_based_training import *
...
def MAS(criterion, att_crit, loader, pre_split, split, opt, model_dir, f_model_dir, previous_category,
        category, log, log_category, freeze_layers=[], b1=False, reg_lambda=1, norm='L2',
    ...
    model_ft = train_model(model_ft, criterion, att_crit, optimizer_ft, loader, split, opt, model_dir,
                           category, log, log_category, resume)

    return model_ft


def MAS_Omega_Accumulation(criterion, att_crit, loader, pre_split, split, opt, model_dir,
                           previous_category, category, log, log_category, freeze_layers=[],
                           b1=False, reg_lambda=1, norm='L2', number=1, previous_ratio=0.5,
                           sub_layers=[], loc_layers=[], rel_layers=[]):
    ...
    if opt['mas_weighted_decay']:
        # model_ft = weighted_decay_accumulate_MAS_weights(criterion, loader, split, opt, model_ft,
        #                                                  freeze_layers, norm, previous_ratio)
        model_ft = weighted_accumulate_MAS_weights(criterion, att_crit, loader, split, opt, model_ft,
                                                   number, freeze_layers, norm, previous_ratio)
    else:
...
def weighted_accumulate_MAS_weights(criterion, att_crit, loader, split, opt, model_ft, number,
                                    freeze_layers=[], norm='L2', previous_ratio=0.5):
    """
    accumulate the importance params: stores the previously computed omega, compute omega on the last
        previous task and accumulate omega resulting on importance params for all the previous tasks
    ...
    # accumulate the new importance params with the previously stored ones (previous omega)
    reg_params = weighted_accumulate_reg_params(model_ft, number, previous_ratio, freeze_layers)
    model_ft.reg_params = reg_params
    sanitycheck(model_ft)
