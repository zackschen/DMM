...
def main(args):
  ...
  for category_i in range(num_category):
    ...
    # checkpoint_category_dir = osp.join(checkpoint_dir, category)
    # if not osp.isdir(checkpoint_category_dir): os.makedirs(checkpoint_category_dir)
    ...
    if category_i == 0:
      ...
      print("begin fisher")
      model.fisher_matrix_diag_origin(loader, category_i, split, mm_crit, att_crit)

    print(category)
    model = base_train.train_model(model, mm_crit, att_crit, loader, split, opt, checkpoint_dir, category_i, category,
                                   log, log_category)
  log.close()
  log_category.close()
