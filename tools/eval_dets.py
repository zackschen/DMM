...
from loaders.dets_flickr_loader import DetsFlickrLoader
...
def evaluate(params):
  ...
  model_prefix = osp.join('output', params['dataset_splitBy'], 'instruments', params['id'])
  ...
  if 'flickr' in params['dataset_splitBy']:
    dets_json = osp.join('cache/detections', params['dataset_splitBy'], '%s_vg_minus_ddpn_%s_dets.json' % \
                         (model_opt['net_name'],  model_opt['tag']))
    data_split = osp.join('cache/prepro', params['dataset_splitBy'], 'data_split.json')
    loader = DetsFlickrLoader(data_h5=data_h5, data_json=data_json, dets_json=dets_json,
  ...
  crit = None
  acc, category_acc, predictions = eval_utils.eval_split(loader, model, crit, split, model_opt)
  ...
  for category in category_acc:
    print('[%s] acc=%.2f%%' % (category, category_acc[category]))
