...
def filter_bboxes(scores, boxes, imdb, nms_thresh, conf_thresh, min_boxes, max_boxes):

    max_conf = np.zeros(boxes.shape[0])
    for cls_ind, class_name in enumerate(imdb.classes[1:]):
        cls_scores = scores[:, cls_ind]
        dets = np.hstack((cls_scores, cls_scores[:, np.newaxis])).astype(np.float32)
...
def main(args):
  ...
  dir_name = 'mask-rcnn-res101-bboxes'
  if 'flickr' in dataset_splitBy:
    IMAGE_DIR = 'data/images/flickr30k/images'
    output_dir = osp.join(args.outdir, dataset_splitBy, dir_name)
  elif 'coco' or 'combined' in dataset_splitBy:
    IMAGE_DIR = 'data/images/mscoco/images/train2014'
    output_dir = osp.join(args.outdir, 'RefCOCO', dataset_splitBy, dir_name)
  ...
  output_dir = osp.join(args.outdir, dataset_splitBy)
  ...
  with open(save_path, 'w') as f:
    json.dump(dets, f)
    """
    filename = osp.join(output_dir, str(image_id)+'.mat')
    boxes, scores = filter_bboxes(scores, boxes, imdb, args.nms_thresh, args.conf_thresh,
                                  args.min_boxes, args.max_boxes)
    print ('boxes', boxes.shape)
    print ('scores', scores.shape)
    exit()
    # sio.savemat(filename, {'pos':proposals, 'category':categorys, 'score':scores})
...
if __name__ == '__main__':
  ...
  parser.add_argument('--outdir', default='/hhd12306-2/xieshuo/feature/', type=str,
  ...
  parser.add_argument('--min_bboxes', default=100, help='min bboxes to be extracted')
  parser.add_argument('--max_bboxes', default=100, help='max bboxes to be extracted')
