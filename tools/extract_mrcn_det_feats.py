...
def det_to_pool5_fc7(mrcn, det, net_conv, im_info):
  ...
  pool5, fc7 = mrcn.box_to_spatial_fc7(Variable(torch.from_numpy(net_conv).cuda()), im_info, box)  # (1, 2048)
  print(fc7.shape)
  exit()
  fc7 = fc7.mean(3).mean(2)
  return pool5, fc7

def det_box_to_pool5_fc7(mrcn, det, net_conv, im_info):
  """
  Arguments:
    det: object instance
    net_conv: float32 (1, 1024, H, W)
...
def main(args):
  ...
  if 'flickr' in dataset_splitBy:
    dets_json = osp.join('cache/detections', dataset_splitBy, '%s_vg_minus_ddpn_%s_dets.json' % (args.net_name, args.tag))
  else:
  ...
  # feats_h5
  # file_name = '%s_vg_minus_ddpn_%s_det_feats.h5' % (args.net_name, args.tag)
  ...
  root_path = '/hhd12306-2/xieshuo/feature/flickr-30k/mask-rcnn-res101'
  ...
  for i, image in enumerate(images):
    ...
    file_name = '%s.h5' % (image['image_id'])
    feats_h5 = osp.join(root_path, file_name)
    f = h5py.File(feats_h5, 'w')
    # det_ids = image['det_ids']
    det_bboxs = image['det_bbox']
    pool5_list, fc7_list = [], []
    # for det_id in det_ids:
      # det = loader.Dets[det_id]
      # det_pool5, det_fc7 = det_to_pool5_fc7(mrcn, det_bbox, net_conv, im_info)
      # det_h5_id = det['h5_id']
      # fc7_set[det_h5_id] = det_fc7.data.cpu().numpy()
      # pool5_set[det_h5_id] = det_pool5.data.cpu().numpy()
    det_pool5, det_fc7 = det_box_to_pool5_fc7(mrcn, det_bboxs, net_conv, im_info)
    f.create_dataset('pool5', dtype='float32', data=det_pool5.cpu().detach().numpy())
    f.create_dataset('fc7', dtype='float32', data=det_fc7.cpu().detach().numpy())
    f.create_dataset('bbox', dtype='float32', data=det_bboxs)
    f.close()
    if i % 20 == 0:
...
if __name__ == '__main__':
  ...
  parser.add_argument('--dataset', type=str, default='flickr30k', help='dataset name: refclef, refcoco, refcoco+, refcocog')
