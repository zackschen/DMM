...
from mas.mas import MAS_Omega_Accumulation
...
def main(args):
  ...
  for category_i in range(num_category):
    ...
    if category_i == 0:
      ...
      model = MAS(mm_crit, att_crit, loader, old_split, split, opt, checkpoint_dir, f_model_dir,
                  old_category, category, log, log_category, freeze_layers, sub_layers=sub_layers,
                  loc_layers=loc_layers, rel_layers=rel_layers)
    else:
      if num_category == 5 and not isDebug: opt["max_category_epoch"] = 20
      previous_ratio = old_data_size * 1.0 / (data_size * 1.0)
      model = MAS_Omega_Accumulation(mm_crit, att_crit, loader, old_split, split, opt, checkpoint_dir,
                                     old_category, category, log, log_category, freeze_layers,
                                     number=category_i, previous_ratio=previous_ratio,
