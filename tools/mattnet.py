...
class MattNet():
  ...
  def __init__(self, args):
    # intialize loader
    data_json = osp.join(ROOT_DIR, 'cache/prepro', args.dataset+'_'+args.splitBy, 'data1.json')
    data_h5 = osp.join(ROOT_DIR, 'cache/prepro', args.dataset+'_'+args.splitBy, 'data1.h5')
    self.loader = GtMRCNLoader(data_h5=data_h5, data_json=data_json)


    # load model
    model_prefix = osp.join(ROOT_DIR, 'output', args.dataset+'_'+args.splitBy, args.model_id, args.category, args.model_id)
    tic = time.time()
    ...
    # loader's feats
    feats_dir = '%s_%s_%s' % (self.model_opt['net_name'], self.model_opt['imdb_name'], self.model_opt['tag'])
    self.loader.prepare_mrcn(head_feats_dir=osp.join(ROOT_DIR, 'cache/feats/', args.dataset+'_'+args.splitBy,
                                                'mrcn', feats_dir), args=args)
    ann_feats = osp.join(ROOT_DIR, 'cache/feats', args.dataset+'_'+args.splitBy, 'mrcn',
                       '%s_%s_%s_ann_feats.h5' % (self.model_opt['net_name'], self.model_opt['imdb_name'],
                                                  self.model_opt['tag']))
    self.loader.loadFeats({'ann': ann_feats})
  ...
  def comprehend_ground(self, image_id, sent_id):
    """
    Arguments:
    - img_data: computed from self.forward_image()
    - expr    : expression in string format
    ...
    - pred_atts  : top 5 attributes, list of (att_wd, score)
    """
    data = self.loader.getImageBatch(image_id, [sent_id], self.model_opt)
    ann_ids = data['ann_ids']
    cxt_ann_ids = data['cxt_ann_ids']
    ...
    tokens = self.loader.decode_labels(labels)

    label = labels[0:1]
    token = tokens[0:1]
    max_len = (label != 0).sum().data[0]
    label = label[:, :max_len]
    expanded_labels = label.expand(len(ann_ids), max_len)
    ...
    rel_ixs = rel_ixs.data.cpu().numpy().tolist()  # (n, )
    rel_ix = rel_ixs[pred_ix]
    cxt_ann_id = cxt_ann_ids[pred_ix]

    ix_to_supcat = self.loader.ix_to_supcat
    ix_to_cat = self.loader.ix_to_cat
    categories, supercategories = [], []
    for ann_id in cxt_ann_id:
      if ann_id == -1:
        supercategories.append('null')
        categories.append('null')
      else:
        print(ann_id)
        print(self.loader.Anns[ann_id]['supercategory_id'])
        print(ix_to_supcat[self.loader.Anns[ann_id]['supercategory_id']])
        supercategories.append(ix_to_supcat[self.loader.Anns[ann_id]['supercategory_id']])
        categories.append(ix_to_cat[self.loader.Anns[ann_id]['category_id']])

    # get everything
    entry = {}
    entry['tokens'] = token
    entry['pred_det_id'] = ann_ids[pred_ix]
    entry['ground_box'] = self.loader.Anns[ann_ids[gd_ix]]['box']
    entry['pred_box'] = self.loader.Anns[pred_ann_id]['box']
    # relative det_id
    entry['rel_ix'] = rel_ix
    entry['rel_ann_ids'] = cxt_ann_ids[pred_ix]
    entry['supercategories'] = supercategories
    entry['categories'] = categories
    entry['rel_det_id'] = cxt_ann_ids[pred_ix][rel_ix]
    entry['rel_box'] = self.loader.Anns[entry['rel_det_id']]['box'] if entry['rel_det_id'] > 0 else [0,0,0,0]
    # attention
    entry['sub_grid_attn'] = sub_grid_attn[pred_ix].data.cpu().numpy().tolist() # list of 49 attn
    entry['sub_attn'] = sub_attn[pred_ix].data.cpu().numpy().tolist() # list of seq_len attn
    entry['loc_attn'] = loc_attn[pred_ix].data.cpu().numpy().tolist() # list of seq_len attn
...
if __name__ == '__main__':
  ...
  parser.add_argument('--category', type=str, default='person', help='category name')
