...
- refs:       [{ref_id, ann_id, box, image_id, split, category_id, supercategory_id, sent_ids, att_wds}]
- images:     [{image_id, ref_ids, file_name, width, height, h5_id}]
- anns:       [{ann_id, category_id, supercategory_id, image_id, box, h5_id}]
...
def prepare_json(refer, sentToFinal, ref_to_att_wds, params, supcat_to_ix):
  # prepare refs = [{ref_id, ann_id, image_id, box, split, category_id, sent_ids}]
  Categories = {cat['id']: cat for cat in refer.data['categories']}
  refs = []
  imageid = []
  sentenceid = []
  trainPersonPount = 0
  ...
  testPersonPount = 0
  Ref_ids = []
  items = refer.Refs.items()
  for ref_id, ref in items:
    box = refer.refToAnn[ref_id]['bbox']
    att_wds = ref_to_att_wds[ref_id] if ref_id in ref_to_att_wds else []
    supercategory_id = supcat_to_ix[Categories[ref['category_id']]['supercategory']]
    split = ref['split']
    
    # if split == "train" and supercategory_id == 5:
    #   if (not ref['image_id'] in imageid) and (trainPersonPount > 5500):
    #     continue
    #   else:
    #     trainPersonPount += len(ref['sent_ids'])
    # if split == "val" and supercategory_id == 5:
    #   if (not ref['image_id'] in imageid) and valPersonPount > 750:
    #     continue
    #   else:
    #     valPersonPount += len(ref['sent_ids'])
    # if split == "testA" and supercategory_id == 5:
    #   if (not ref['image_id'] in imageid) and testPersonPount > 1500:
    #     continue
    #   else:
    #     testPersonPount += len(ref['sent_ids'])

    # if supercategory_id in [0,3] :
    #   continue

    refs += [{'ref_id': ref_id, 'split': ref['split'], 'category_id': ref['category_id'],
              'supercategory_id':supercategory_id,'ann_id': ref['ann_id'], 'sent_ids': ref['sent_ids'], 'box': box,
              'image_id': ref['image_id'], 'att_wds': att_wds} ]
    Ref_ids.append(ref_id)
    sentenceid+=ref['sent_ids']
    imageid.append(ref['image_id'])
  print('There in all %s sentence.' % len(sentenceid))
  ...
  for image_id, image in refer.Imgs.items():
    ...
    images += [ {'image_id': image_id, 'height': height, 'width': width, 'file_name': file_name, 'ref_ids': new, 'ann_ids': ann_ids, 'h5_id': h5_id} ]
    h5_id += 1
  ...
  for image_id in refer.Imgs:
    ...
    for ann_id in ann_ids:
      ...
      anns += [{'ann_id': ann_id, 'category_id': ann['category_id'],
                'supercategory_id':supcat_to_ix[Categories[ann['category_id']]['supercategory']],
                'box': ann['bbox'], 'image_id': image_id, 'h5_id': h5_id}]
      h5_id += 1
...
def main(params):
  ...
  supercategory = list(set([cat['supercategory'] for cat in refer.data['categories']]))
  supcat_to_ix = {w: i for i, w in enumerate(supercategory)}
  category_structure = {}
  for supcat in supercategory:
    category_structure[supcat] = []
  for cat in refer.data['categories']:
    category_structure[cat['supercategory']].append(cat)
  ...
  # and write json
  refs, images, anns, sentences = prepare_json(refer, sentToFinal, ref_to_att_wds, params, supcat_to_ix)
  json.dump({'refs': refs,
             'images': images,
             'anns': anns,
             'sentences': sentences,
...
if __name__ == '__main__':
  ...
  parser.add_argument('--output_json', default='10task.json', help='output json file')
  parser.add_argument('--output_h5', default='10task.h5', help='output h5 file')
  parser.add_argument('--data_root', default='/mnt/data1/chencheng/COCOImage/', type=str, help='data folder containing images and four datasets.')
